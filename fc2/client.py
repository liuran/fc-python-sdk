...
class Client(object):
    ...
    def _check_function_param_valid(self, codeZipFile, codeDir, codeOSSBucket, codeOSSObject):
        ...
        if codeOSSBucket:
            if not codeOSSObject:
                return False, 'codeOSSBucket and codeOSSObject must to exist at the same time'
            code_d['oss'] = (codeOSSBucket, codeOSSObject)

        if len(code_d) == 0:
            return False, 'codeZipFile, codeDir, (codeOSSBucket, codeOSSObject) , these three parameters must have an assignment'

        if len(code_d) > 1:
            return False, 'codeZipFile, codeDir, (codeOSSBucket, codeOSSObject) , these three parameters need only one parameter assignment'
    ...
    def create_function(
        ...
        """
        ok, errmsg = self._check_function_param_valid(codeZipFile, codeDir, codeOSSBucket, codeOSSObject)
        if not ok:
